...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    let sum_a = thread::spawn(move || a.into_iter().sum());
    let sum_b = thread::spawn(move || b.into_iter().sum());
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    thread::scope(|scope| {
        let sum_a = scope.spawn(|| a.iter().sum());
        let sum_b = scope.spawn(|| b.iter().sum());
        (sum_a.join().unwrap(), sum_b.join().unwrap())
    })
}
